...
@pytest.mark.django_db
@patch('accounts.views.captcha_views.verify_recaptcha', return_value=True)
...
@pytest.mark.django_db
@patch('accounts.views.captcha_views.verify_recaptcha', return_value=False)
...
def test_verify_recaptcha_returns_false_on_request_exception():
    ...
    with override_settings(RECAPTCHA_SECRET_KEY='secret'):
        with patch('accounts.views.captcha_views.requests.post', side_effect=requests.RequestException) as mock_post:
...
def test_verify_recaptcha_returns_false_when_api_fails():
    ...
    with override_settings(RECAPTCHA_SECRET_KEY='secret'):
        with patch('accounts.views.captcha_views.requests.post', return_value=ApiFailureResponse()) as mock_post:
            assert verify_recaptcha('token') is False
        mock_post.assert_called_once()


@pytest.mark.django_db
@patch('accounts.views.auth.verify_recaptcha', return_value=False)
