...
| `Subscription` | status `trialing\|active\|past_due\|canceled`, period_start/end, gateway, external_customer_id, external_subscription_id | FK org, plan. `unique(org) WHERE status IN (active, trialing)`. **As built (It9)**: trial-only slice — `billing.Subscription` OneToOne org, `plan_key`, status `trialing\|expired`, `trial_ends_at` + notice stamps; catalog stays the static `PLANS` dict. Gateway fields arrive with Wompi. |
...
| I13 | Plan limits are checked at action time (create project, invite, upload) against `Subscription.plan.limits`. 30-day free retention is an ACCESS restriction, never deletion (DP-04). **Effective plan (It9)**: console override (`Organization.plan != 'free'`) > active trial > free — resolved lazily at read time by `billing.services.effective_plan`. | Service guards (`enforce_limit(org, 'max_active_projects')`) + tests. | F1 |
